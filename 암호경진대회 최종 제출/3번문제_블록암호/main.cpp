...
using namespace std;
Byte  answer[16];
Byte IV_null[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
Byte P[16];
...
int main()
{
	...
	struct CTX cry;
	cry.cipher_length = 16;
	for (int i = 0; i < 29; i++) {
		memcpy(IV_null, IV, 16);
		memset(P, 0, 16);
		Byte* crypted = c1 + i * 16;
		cry.ciphertext = crypted;
		cry.IV = IV_null;
		...
		for (int j = 15; j >= 0; j--) {
			...
			for (; (unsigned int) t < 0x00FF + 1; t++) {
				IV_null[j] = IV[j] ^ t;
				
				if (Dec_CTX(&cry) == 0) break;
			}
		...
		for (int j = 0; j < 16; j++) printf("%c", P[j]);
		for (int j = 0; j < 16; j++) {
			IV[j] = crypted[j] ^ P[j];
		}
